...
其中：
- `supported_classes` 和 `supported_targets` 內容相同，兩個欄位都保留，方便外部系統直接取用
- `target_supported_modes` 會告訴你每個類別支援 `suction` 或 `grasp`
- `target_metadata` 會帶該類別目前在場景裡的 source / destination / support z 等資訊
...
把位置座標從現實側 frame 轉成 `0422` 虛擬世界座標，但不真的移動任何東西。
...
欄位說明：
- `position` / `xyz`: `[x, y, z]`
- `coordinate_frame`: 預設 `sim_world_m`
- `units`: 只在 `delta_xyz_*` frame 需要時使用，預設 `auto`
...
注意：
- 如果 `coordinate_frame=sim_world_m`，座標才是 `0422` 世界座標公尺值
- 如果 `coordinate_frame=delta_xyz_*` 或 `conveyor_semantic_m`，後端會先轉成 `0422` 再移動
...
現在 `object_pose_overrides` 裡也會附上：
- `coordinate_frames`
- `object_position_semantics`

注意：
- `coordinate_frames.delta_xyz_*` 同樣是從 override 的 sim pose 反推的 coarse 參考值
- 如果外部系統需要精確 real-to-sim 對應，應以 `POST /position_convert` 的 forward conversion 為主
...
## 9. Coordinate Conversion Regression
...
- `GET /ee_pose` 回傳 `ee_coordinate_frames / tool_coordinate_frames`
- `GET /scene_object_poses` 回傳 `coordinate_frames`
- `GET /object_poses` 回傳 `coordinate_frames`
